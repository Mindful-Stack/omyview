...
Properties this must achieve: (a) `Array.prototype.sort` in QML's JS engine is not guaranteed stable, which is why `order` is part of the comparator — `test_equal_scores_keep_input_order` proves the ordering rather than assuming it; do not remove `order` from the comparator. (b) `fuzzyScore` must return the maximum over all alignments — `test_best_alignment_not_first_occurrence` and `test_repeated_characters_pick_the_run` both fail under the greedy variant, so if either passes vacuously the DP is not being exercised.

- [ ] **Step 4: Run the tests to verify they pass**

Run: `cd ~/Source/omyview && QT_QPA_PLATFORM=offscreen /usr/lib/qt6/bin/qmltestrunner -input tests/tst_find.qml`
Expected: `Totals: 12 passed, 0 failed`. Then `mise run test` — everything else still green.
...
Pure function in logic.js, Tier 1 tested. Greedy subsequence with consecutive and word-start
bonuses, a class bonus so the app outranks a tab that merely mentions it, a length penalty for
...
Run: `QT_QPA_PLATFORM=offscreen /usr/lib/qt6/bin/qmltestrunner -input tests/tst_find.qml`
Expected: `Totals: 15 passed, 0 failed`.
