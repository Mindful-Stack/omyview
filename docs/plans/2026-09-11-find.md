...
// Greedy left-to-right: each query character takes its first occurrence after the previous
// match. Per character: 1, +2 when it directly follows the previous match, +3 at a word start.
function fuzzyScore(needle, hay) {
    var score = 0, prev = -2, from = 0
    for (var i = 0; i < needle.length; i++) {
        var at = hay.indexOf(needle.charAt(i), from)
        if (at < 0) return null
        score += 1
        if (at === prev + 1) score += 2
        if (_wordStart(hay, at)) score += 3
        prev = at; from = at + 1
    }
    return score - hay.length * FIND_LENGTH_PENALTY
}
...
Property this must achieve: `Array.prototype.sort` in QML's JS engine is not guaranteed stable, which is why `order` is part of the comparator — `test_equal_scores_keep_input_order` proves the ordering rather than assuming it. Do not remove `order` from the comparator.

- [ ] **Step 4: Run the tests to verify they pass**

Run: `cd ~/Source/omyview && QT_QPA_PLATFORM=offscreen /usr/lib/qt6/bin/qmltestrunner -input tests/tst_find.qml`
Expected: `Totals: 10 passed, 0 failed`. Then `mise run test` — everything else still green.
...
Run: `QT_QPA_PLATFORM=offscreen /usr/lib/qt6/bin/qmltestrunner -input tests/tst_find.qml`
Expected: `Totals: 13 passed, 0 failed`.
...
```qml
                Keys.onPressed: function (e) {
                    ...
                    if (e.key === Qt.Key_Backspace) {
                        if (finding) root.setQuery((e.modifiers & Qt.ControlModifier) ? "" : root.query.slice(0, -1))
                        return
                    }
                    ...
                    if (chord) return                       // Ctrl/Alt/Meta+key: reserved for future actions
                    var next = Logic.appendQueryText(root.query, e.text)
                    if (next !== root.query) root.setQuery(next)
                }
```

Property: every branch accepts the event (set once at the top), so nothing bubbles to the Flickable — before this change unhandled keys were left unaccepted, which was harmless only because nothing consumed them. Shift is deliberately not in `chord`: Shift+letter is an uppercase letter, Shift+Tab is `Key_Backtab`.
...
Run: `bash tests/ui/run.sh`
Expected: `test_match_outline_and_dim_follow_the_roles` fails at `"tiles carry a matchOutline item"`; the other two fail on `view.testBar` being undefined.
...
```qml
    ...
    // while this tile is the drop target (that border takes precedence). No mouse handling.
    Rectangle {
        objectName: "matchOutline"
        anchors.fill: parent
        visible: tile.matched && !tile.dropTarget
        color: "transparent"
        radius: 5
...
// glyph, the query, and "n of m". No key handling and no focus: Overview's keyCatcher stays
// the single focus item (docs/specs/2026-09-11-find-design.md).
Item {
    id: bar
    property string query: ""
...
Run: `mise run test`
Expected: all green, including the three new `Find::` tests. If `test_match_outline_and_dim_follow_the_roles` fails on opacity: `motion.scale` is 0 in `init()`, so the `Behavior` is disabled and the value is immediate — a failure means the binding, not timing.
