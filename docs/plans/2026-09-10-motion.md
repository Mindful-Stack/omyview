...
- Qt `Behavior` semantics, **as observed in the offscreen suite during Task 6** (they contradicted this plan's original assumption): disabling a `Behavior` does *not* stop a transition already in flight, and `MouseArea.drag` writes the target's position from C++ without removing the QML binding, so the in-flight transition's next tick re-asserts the property. A plain JS assignment (`x = x - dx`) does detach the binding. Task 6's tile glide design follows from this. What did hold (re-verified in the final review with minimal `qmltestrunner` cases on Qt 6.11): a *QML* write through a disabled `Behavior` does stop the in-flight transition and lands directly — it is only C++-side writes (`QQuickItem::setX` from `MouseArea.drag`) that bypass the property system and so neither stop it nor detach the binding; a write equal to the current value with no animation running writes directly; an identical-target write during a running animation is ignored; and `ListModel.set` with byte-identical values emits no `dataChanged`, so an identical re-`set` can never re-assert a binding.
- Three more Qt facts, **as observed in the offscreen suite during Task 7** shaping the tile-appear animation:
  - A manual write to a property still gated by an *enabled* `Behavior` (e.g. `appearScale = 0.9` while `!appearAnim.running` alone gates the hover/lift Behavior) is caught and smoothed by that Behavior instead of landing as a hard write.
  - Restarting the animation before writing the start value doesn't help either: with no explicit `from:`, `restart()` captures `from` from the property's current value at that instant, so a restart-then-write ordering produced a no-op ramp (`from` captured as 1).
  - A `NumberAnimation` with an explicit `from:` writes that value to the target property *before* flipping the animation's own `running` flag to true — so a Behavior gated only on `!appearAnim.running` is still enabled at that instant and will catch the write; a separate flag (`priming`) is needed to close that one-tick gap.
...
> **Revised during execution (2026-09-10).** The brief's version had `appear()` write
> `appearScale = 0.9; appearOpacity = 0` itself before calling `appearAnim.restart()`, with the
> Behaviors below gated on `!appearAnim.running` alone (no `priming`). Those direct writes went
> straight through the hover/lift Behaviors on `scale`/`opacity` — which were still enabled at
> that instant — and got smoothed into their own transition instead of landing, so the tile eased
> gently off 1/1 instead of snapping to the 0.9/0 start before the appear ramp took over. Dropping
> the manual writes and giving both `NumberAnimation`s an explicit `from:` (0.9 and 0) looked like
> the fix — Qt writes `from` to the target property before the animation's own `running` flips
> true — but a restart-then-write ordering tried in between produced a no-op ramp, because by the
> time the animation restarted, `from` had already been captured as the tile's current value of 1.
> The shipped code below carries explicit `from:` values *and* a `priming` flag that disables the
> hover/lift Behaviors for that one instant: `from` still lands on `appearScale`/`appearOpacity`
> before `running` goes true, but with `priming` also true the Behaviors gated on it do not catch
> that write, so it lands unsmoothed and the appear ramp takes over cleanly from there.
...
In the Overview tile delegate add, next to `Component.onDestruction`:
