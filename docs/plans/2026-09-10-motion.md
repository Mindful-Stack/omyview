...
- Qt `Behavior` semantics used below (qquickbehavior.cpp, Qt 6): a write while the Behavior is **disabled** stops any running behavior animation and writes directly; a write with a value equal to the current one, with no animation running, writes directly without animating; a write while an animation is already running towards the same target is ignored. The plan's tests are written so that a wrong assumption here fails a test rather than passing silently (see Task 6).
...
```qml
    ...
    function test_tile_settles_by_gliding_after_release() {
        ...
        dragBy(35, 20)
        // The optimistic wx is the drop point round-tripped through window coordinates, so it
        // can differ from dropX by a fraction of a pixel; the glide has not ticked yet.
        fuzzyCompare(t.x, before + 35, 0.5, "held at the drop point")
        ...
        verify(t.x > before + 30, "glide starts from the drop point, x=" + t.x)
        wait(80)
        verify(t.x > before + 1 && t.x < before + 34, "half-way, x=" + t.x)
        wait(200)
        compare(t.x, before)
    }
...
In the `WindowTile` delegate (`Overview.qml:582-604`), after `titleSize: root.captionSize` add:
...
```qml
                            ...
                            Behavior on x      { enabled: root.layoutMotion && !windowTile.dragging
                                NumberAnimation { duration: root.motion.normal; easing.type: root.motion.move } }
                            Behavior on y      { enabled: root.layoutMotion && !windowTile.dragging
                                NumberAnimation { duration: root.motion.normal; easing.type: root.motion.move } }
...
In `dragArea.onPressed` (lines 615-627) move the two `root.*` assignments **above** `beginGrab`, so the Behaviors are already disabled when `beginGrab` may offset x/y for a re-grab mid-release:
...
Property: `beginGrab` reads `ghost.xScale` *before* the lift animation ticks, so its compensation is unchanged by the reorder (the existing `test_regrab_*` pins this). In `endDrag()` the order is already right: `draggingAddress = ""` (Behaviors on) **then** `tile.restoreDrag()` (x/y rebound → the settle/snap-back glides).
...
Run: `mise run test`
Expected: green. If `test_dragged_tile_*` fails at "the interrupted glide never writes again", the Behavior did not stop its running animation on the disabled write; the fix then is, in `onPressed`, to force one write through the *enabled* Behavior before disabling it (`windowTile.x = windowTile.x + 0.0001` is not enough, as equal-target writes are ignored — write the current model value: `windowTile.x = model.wx` immediately before `root.draggingAddress = ...`). Do not weaken the test.
...
settle place rather than glide) and, for the dragged tile, on `!dragging` — the drag writes x/y
straight through and stops any glide still running. Boxes are a reconciled `ListModel`
